...
export class AIXReader {
                ...
                zipReader.getEntries(async (entries) => {
                    if (entries.length) {
                        try {
                            ...
                            const fileSize = source instanceof Blob ? source.size : 0;
                            const extensions = await this.generateExtensions(jsonEntries, fileSize);
                            zipReader.close();
     ...
     */
    static async generateExtensions(entries, fileSize = 0) {
        const buildInfos = [];
        const descriptors = [];
        const extensions = [];
            ...
            if (Array.isArray(buildInfo.info)) {
                for (let i = 0; i < buildInfo.info.length; i++) {
                    ...
                    if (descriptor) {
                        ...
                        extensions.push(new AIExtension(info.type, desc, info, fileSize, buildInfo.name));
                    }
                }
            } else {
                const descriptor = descriptors.find(d => d.name === buildInfo.name);
                if (descriptor) {
                    const desc = Array.isArray(descriptor.descriptor)
                        ...
                        : descriptor.descriptor;
                    extensions.push(new AIExtension(buildInfo.info.type, desc, buildInfo.info, fileSize, buildInfo.name));
                }
            }
        }
    ...
    static getExtensionFolder(entry) {
        ...
        // Typical structure: 
        // 1. com/domain/Extension/files/component_build_infos.json
        // 2. com/domain/Extension/components.json
        // We want 'com/domain/Extension' as the ID to match them.
        // Actually, let's return the full path up to the file name, minus 'files/' if present.

        // Simpler approach: return the directory containing the file, 
        // stepping back if it's the 'files' directory.

        let path = parts.slice(0, -1); // remove filename
        if (path.length > 0 && path[path.length - 1] === 'files') {
            path.pop(); // remove 'files'
        }

        // This should return "com/domain/Extension" which is unique enough for matching
        // descriptors and build_infos within the same extension package.
        return path.join('/');
    }
}
