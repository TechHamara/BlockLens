...
Blockly.Blocks['helpers_dropdown'] = {
  ...
  domToMutation: function (xml) {
    // Force the helper red color (in case it was inherited from parent block)
    this.setColour(Blockly.COLOUR_HELPERS);
    ...
    var optionList = db ? db.getOptionList(this.key_) : null;

    // If optionList is not found, use fallback rendering with value from XML
    if (!optionList) {
      // Fallback: show key as tag and value in a simple dropdown
      var tag = this.key_ || 'Option';
      var options = value ? [[value, value]] : [['', '']];
      ...
      if (value) {
        try { this.setFieldValue(value, 'OPTION'); } catch (e) { }
      }
      return;
    }
    ...
    var defaultOpt = optionList.defaultOpt;
    if (!defaultOpt && optionList.options && optionList.options.length > 0) {
      defaultOpt = optionList.options[0].name;
    }

    var finalValue = value || defaultOpt;
    if (finalValue) {
      this.setFieldValue(finalValue, 'OPTION');
    }
  },
